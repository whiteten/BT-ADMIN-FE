...
### 코드 품질 규칙 (SonarQube 정적분석 기반 — 생성 시 준수하여 스멜 예방)

> SonarQube 상위 지적 룰에서 도출. **작성 시점에 지켜 사후 수정(회귀 위험)을 없앤다.** 취약점(vulnerability)은 0건이고 대부분 유지보수성 스멜이므로, 아래는 "동작 변경 없이 깨끗하게 짜는" 규칙이다.

- **중첩 삼항 금지 (S3358)** — 가장 많이 걸림: JSX·로직에서 삼항연산자를 중첩하지 말 것. `if/else`, 조기 return, 지역변수, 또는 매핑 객체(`STATUS_LABELS[x]`)로 분리. JSX 조건부는 `조건 ? <A/> : 조건2 ? <B/> : <C/>` 대신 별도 변수나 IIFE/헬퍼 컴포넌트로.
- **컴포넌트 안에서 컴포넌트 정의 금지 (S6478)** — 렌더 함수/부모 컴포넌트 본문 안에 `function Sub()`·`const Sub = () => ...` 정의 금지(매 렌더 재생성 → 성능·상태손실). 컴포넌트는 모듈 최상위로 hoist하고 props로 전달.
- **React 리스트 key에 배열 index 금지 (S6479)** — `.map((item, i) => <X key={i}/>)` 금지. `key`는 항목의 안정적 고유값(`item.id`·`menuKey`·PK 조합) 사용. 고유 id 없으면 `createUUID()`로 1회 생성해 데이터에 보관.
- **불필요한 타입 단언/non-null 제거 (S4325)** — 의미 없는 `as T`·`value!`(non-null assertion) 남발 금지. 타입 좁히기(guard)·옵셔널 체이닝으로 처리. 정말 필요한 단언만 남기고 근거를 주석.
- **컴포넌트 복잡도 제한 (S3776)** — 한 컴포넌트/함수에 분기·중첩을 쌓지 말고 하위 컴포넌트·커스텀 훅·헬퍼로 분할.
- **미사용 import/변수/dead store 제거 (S1128, S1854)** — 미사용 import·변수, 다시 읽히지 않는 대입은 남기지 않음. 코드 수정 후 `npx eslint --fix <file>` 실행(기존 규칙).
- **중첩 템플릿 리터럴 지양 (S4624)** — 백틱 안에 백틱 중첩하지 말고 지역변수로 분리.
