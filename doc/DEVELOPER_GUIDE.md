...
routes.tsx는 위에서 아래로 세 블록으로 구성됩니다.

1. **import** — `react`, `react-router-dom`, `features/router/`의 라우팅 보조 모듈, `NotFound`
2. **lazy 페이지 선언** — 모든 페이지를 `React.lazy`로 선언
3. **`routes` 배열 export** — 라우트 트리
...
- 실제 페이지를 그리는 leaf는 `{ path: '<page>', element: <Page /> }`입니다.
...
- 탭 레이아웃이 필요한 상세 페이지는 `element`에 레이아웃 컴포넌트(`<Feature>DetailLayout`)를 두고, `children`에 탭·하위 라우트를 중첩합니다.
...
- **화면 커스터마이징(variant)**: variant를 갖는 path는 element를 `<DynamicElement>`로 감쌉니다. → 19장 참조
- **queryString 분기**: 같은 path를 queryString으로 분기하는 메뉴는 `handle.queryParams`를 선언합니다. → `queryString 기반 메뉴 분기 가이드` 참조
