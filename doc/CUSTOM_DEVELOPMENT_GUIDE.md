...
> ⚠️ **일시 제외(2026-07 빌드 체계 전환)**: 구 `create-custom` 스크립트는 nx 기반이라 turborepo+Rsbuild 전환 시 미이관 상태입니다. 실수요 발생 시 전환 커밋 이전 이력(`scripts/create-custom.js`)에서 복원·개정 예정이며, 그때까지는 아래 Step 1~4(키 확인 → 사본 복사 → exposes 등록 → site-manifest 등록)를 수동으로 진행합니다.

구 스크립트가 자동화했던 작업 (복원 시 참고):
...
> 동기화 여부는 구 `create-custom --check`가 검증하던 항목(키 1:1 대조 + routes.tsx의 pv 키 실존 + 노출 모듈 파일 존재 확인)을 수동 대조로 확인합니다 (스크립트는 일시 제외 — 위 "스캐폴딩 스크립트" 참조).
...
| 커스텀을 적용했는데 표준 화면이 나옴 | exposes 키와 화면 키 불일치(`./` prefix 포함 여부 확인). 콘솔에 `custom remote에 '...'가 노출되어 있지 않아` 경고가 찍힘. exposes·site-manifest·routes pv 키를 수동 대조(구 create-custom `--check` 항목) |
...
# (구) scripts/create-custom.js — 오버라이드 스캐폴딩·정합성 검증. 빌드 체계 전환으로 일시 제외,
#     전환 커밋 이전 이력에서 복원 예정 ("스캐폴딩 스크립트" 절 참조)
```
