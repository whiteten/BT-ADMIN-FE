...
| dev 프록시 | [apps/host/proxy.config.js](../apps/host/proxy.config.js) | 개발 시 `/remotes/custom` 요청을 custom dev 서버(4211)로 우회 |
...
아래 Step 1~4(키 확인 → 사본 복사 → exposes 등록 → site-manifest 등록)는 스크립트로 자동화되어 있습니다.

```bash
pnpm run create-custom                                  # 대화형 — 앱·화면 번호 선택, 복사 미리보기 후 확인
pnpm run create-custom fca bot-config/bot/list [라벨]   # 비대화형 — 인자 전달 시 즉시 실행
pnpm run create-custom fca bot-config/bot/list --dry-run  # 복사 대상 파일 목록만 출력
pnpm run create-custom --check                          # exposes·site-manifest·routes 키 정합성 검증
```

- 대상 remote의 `routes.tsx`에서 `pv()` 화면 키를 추출해 선택 목록으로 제시하므로 키를 외우거나 오타 낼 일이 없습니다.
- 원본 페이지부터 **상대 경로 import 그래프를 재귀 추적**해 의존 feature 파일·에셋까지 한 번에 미러링 복사합니다 (`@` 별칭 import는 복사 대상 아님).
- exposes(`./` prefix)·site-manifest 양쪽에 같은 키를 자동 등록합니다.
- **이미 존재하는 파일은 덮어쓰지 않고 건너뜁니다** (현장 수정분 보호). 원본 최신화가 필요하면 해당 사본을 직접 삭제 후 재실행하세요.

실행 후에는 Step 5(로컬 확인)부터 진행하면 됩니다. 아래 Step 1~4는 스크립트가 하는 일의 원리이자 수동 절차입니다.
...
> 동기화 여부는 `pnpm run create-custom --check`로 검증할 수 있습니다 (키 1:1 대조 + routes.tsx의 pv 키 실존 + 노출 모듈 파일 존재 확인).
...
# 터미널 2 — custom remote (별도 실행, 포트 4211)
npx turbo run dev --filter=@bridgetec/ui-remote-custom
```

- `pnpm serve`(serve-host.js)는 custom을 지원하지 않으므로 **custom은 반드시 별도 터미널**로 띄웁니다.
- host의 dev 프록시가 `/remotes/custom` 요청을 4211로 우회합니다. custom 서버를 안 띄우면 HEAD 체크가 실패해 표준으로 동작합니다(평소 개발에 영향 없음).
...
```bash
npx turbo run build --filter=@bridgetec/ui-remote-custom
...
└── services/bff/
    └── conf/                        ← 컨테이너 /app/svc-conf 로 read-only 마운트
        ...
        └── remotes/custom/          ← custom 번들 배치 위치 (apps/custom/dist/* 전체)
            ├── remoteEntry.js
            └── *.js ...
```
...
1. 현장 개발 PC에서 빌드: `npx turbo run build --filter=@bridgetec/ui-remote-custom` → `apps/custom/dist/`
...
- **`apps/custom/rsbuild.config.ts`의 `{ consumeOnly: true }` 옵션을 절대 제거하지 마세요.** (구현: [tools/rsbuild/remote-config.ts](../tools/rsbuild/remote-config.ts)) 일반 remote처럼 공급자로 합류하면 React 인스턴스가 2개가 되어 앱 전체가 깨집니다(Invalid hook call). 상세한 이유는 해당 파일 주석 참조.
...
| 커스텀을 적용했는데 표준 화면이 나옴 | exposes 키와 화면 키 불일치(`./` prefix 포함 여부 확인). 콘솔에 `custom remote에 '...'가 노출되어 있지 않아` 경고가 찍힘. `pnpm run create-custom --check`로 키 정합성 일괄 점검 |
| `Invalid hook call` / `useMemoCache` 에러 | consume-only 설정이 깨짐 — `apps/custom/rsbuild.config.ts`의 `{ consumeOnly: true }` 변경 여부 확인 |
| 런타임 로드 실패 경고 후 표준 fallback | 오버라이드가 제품에 없는 공유 모듈을 사용. import 목록 점검 |
| dev에서 HEAD 체크가 계속 404 | custom dev 서버(4211) 미기동, 또는 `apps/host/proxy.config.js`의 `/remotes/custom` 프록시 설정 확인 |
...
├── rsbuild.config.ts                  # consumeOnly 설정 (수정 금지)
└── src/app/
...
apps/host/
├── proxy.config.js                                        # dev: /remotes/custom → 4211
...
scripts/create-custom.js                   # 오버라이드 스캐폴딩·정합성 검증 (pnpm run create-custom)
```
