/**
 * 교환기 업무시간 테넌트 카드 (240×100) — CtiCodeTenantCard/AdnTenantCard 패턴.
 *
 * 표시: 테넌트명 + 업무시간(마스터) 수 / 시간대(슬롯) 수.
 * 통계는 별도 API 없이 목록 데이터를 페이지에서 group by 하여 주입.
 */
import { Building2 } from 'lucide-react';

export interface PbxWorktimeTenantCardStats {
  worktimeCnt: number;
  slotCnt: number;
}

interface PbxWorktimeTenantCardProps {
  tenantName: string;
  stats: PbxWorktimeTenantCardStats;
  selected: boolean;
  onClick: (e: React.MouseEvent<HTMLDivElement>) => void;
}

export default function PbxWorktimeTenantCard({ tenantName, stats, selected, onClick }: PbxWorktimeTenantCardProps) {
  return (
    <div
    ...
    >
      ...
      <div className="flex-1 flex flex-col gap-0.5 text-xs text-gray-600">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="font-medium text-blue-600">{stats.slotCnt.toLocaleString()}</span>
        </div>
      </div>
    </div>
  );
