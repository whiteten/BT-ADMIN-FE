/**
 * 스킬셋 테넌트 카드 (240×100) — AdnTenantCard 패턴.
 */
import { Building2 } from 'lucide-react';

export interface SkillsetTenantCardStats {
...
interface Props {
  ...
  tenantName: string;
  stats: SkillsetTenantCardStats;
  selected: boolean;
  onClick: (e: React.MouseEvent<HTMLDivElement>) => void;
}

export default function SkillsetTenantCard({ tenantId, tenantName, stats, selected, onClick }: Props) {
  const isAll = tenantId === null;
  const { skillsetCount, groupCount, unassignedCount } = stats;
  return (
    <div
    ...
    >
      ...
      <div className="flex-1 flex flex-col gap-0.5 text-xs text-gray-600">
        <div className="flex items-center justify-between">
          <span className="text-gray-500">스킬셋</span>
          <span className="font-semibold text-gray-800">{skillsetCount.toLocaleString()}건</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-gray-500">업무그룹</span>
          <span className="font-medium text-blue-600">{groupCount.toLocaleString()}</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-gray-500">미배정</span>
          <span className={`font-medium ${unassignedCount > 0 ? 'text-red-500' : 'text-gray-400'}`}>{unassignedCount.toLocaleString()}</span>
        </div>
      </div>
    </div>
  );
