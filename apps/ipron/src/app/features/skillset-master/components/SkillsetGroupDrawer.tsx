...
interface Props {
  ...
  parent?: SkillsetGroupResponse | null;
  group?: SkillsetGroupResponse | null;
  onCancel: () => void;
  onSubmit: (req: SkillsetGroupCreateRequest | SkillsetGroupUpdateRequest) => void;
  loading?: boolean;
}
...
export default function SkillsetGroupDrawer({ open, mode, tenantId, parent, group, onCancel, onSubmit, loading }: Props) {
  ...
  const handleFinish = (values: FormValues) => {
    if (mode === 'create') {
      if (!tenantId) return;
      const req: SkillsetGroupCreateRequest = {
        tenantId,
      ...
      const req: SkillsetGroupUpdateRequest = {
        treeName: values.treeName,
        sortSeq: values.sortSeq ?? 0,
      };
      onSubmit(req);
