...
export default function AgentMasterTable({
  ...
  const columnDefs: ColDef<AgentResponse>[] = useMemo(
    () => [
      ...
      { headerName: '그룹', field: 'groupName', flex: 1.1, minWidth: 100, valueFormatter: (p) => p.value ?? '-' },
      {
        headerName: '로그인 ID',
        field: 'agentLoginId',
        flex: 1,
      ...
      { headerName: '상담사명', field: 'agentName', flex: 1, minWidth: 90 },
      {
        headerName: '상담등급',
        field: 'agentGrade',
        flex: 0.9,
      ...
      { headerName: '작업일시', field: 'workTime', flex: 1.4, minWidth: 130, valueFormatter: (p) => p.value ?? '-' },
      {
        headerName: '',
        maxWidth: 60,
        sortable: false,
