 ...
 * 상담사 엑셀 가져오기 Drawer.
 *
 * - 테넌트/그룹 고정. 엑셀 양식은 export 와 동일한 컬럼.
 * - 백엔드는 비동기 처리. FE 는 1초 polling 으로 진행률 갱신.
 * - 한 행 실패해도 나머지는 계속 진행, 실패 행은 실시간 누적 표시.
 *
 * DN 엑셀 가져오기 Drawer (DnImportDrawer) 패턴 1:1 이식.
 */
...
interface AgentImportDrawerProps {
  open: boolean;
...
export default function AgentImportDrawer({ open, tenantId, groupId, tenantName, groupName, onClose, onSuccess }: AgentImportDrawerProps) {
  ...
  const startPolling = (taskId: string) => {
    ...
    pollTimer.current = setInterval(async () => {
      try {
        const status = await agentMasterApi.getImportStatus(taskId);
        setProgress(status);
        if (status.done) {
          if (pollTimer.current) {
            clearInterval(pollTimer.current);
        ...
        console.error('[agent-import] status polling failed', e);
      }
    }, 1000);
  };

  const handleStart = async () => {
    if (!tenantId || !groupId || !file) return;
    setPhase('uploading');
    setErrorMsg(null);
    try {
      const { taskId } = await agentMasterApi.startImport({ tenantId, groupId, file });
  ...
  return (
    <Drawer
      ...
      footer={
        <div className="flex justify-end gap-2">
          ...
          {phase !== 'done' && (
            <Button type="primary" onClick={handleStart} disabled={!tenantId || !groupId || !file || inProgress} loading={inProgress}>
              가져오기 시작
            </Button>
          )}
        </div>
    ...
    >
      <Alert
        ...
        showIcon
        className="!mb-4"
        message={`${tenantName ?? '-'} / ${groupName ?? '-'} 로 일괄 등록됩니다`}
        description={
          <div className="space-y-1 text-xs">
            <div>엑셀 양식은 내보내기와 동일한 컬럼입니다 (테넌트/그룹 컬럼은 무시되고 위 테넌트/그룹으로 강제 등록).</div>
            <div>이미 존재하는 로그인 ID, 누락된 필수값은 실패 처리되며 나머지는 계속 진행됩니다.</div>
            <div className="text-gray-500">진행률은 1초마다 갱신됩니다.</div>
          </div>
        }
      />
      ...
      {progress && (phase === 'polling' || phase === 'done' || phase === 'uploading') && (
        <div className="mt-4 p-4 border border-gray-200 rounded-md bg-white space-y-3">
              ...
              <div className="max-h-[200px] overflow-y-auto text-[11px] space-y-1">
                {progress.failed.map((f, idx) => (
                  <div key={idx} className="flex gap-2">
                    ...
                    <span className="text-gray-700 shrink-0">{f.agentLoginId || '-'}</span>
          ...
          {errorMsg && <div className="text-xs text-red-600 border-t border-gray-100 pt-3">{errorMsg}</div>}
        </div>
      )}
    </Drawer>
  );
