...
const ScenarioVersionSheet = forwardRef<ScenarioVersionSheetRef, ScenarioVersionSheetProps>(({ serviceId, serviceName }, ref) => {
  ...
  const { mutate: downloadScenarioMutate } = useDownloadScenario({
    mutationOptions: {
      onSuccess: (blob, variables) => {
        const fileName = editingVersion?.scenarioFile ?? `scenario_v${(variables as Record<string, unknown>).serviceVer}.sxml`;
  ...
  const { mutate: downloadDocMutate } = useDownloadScenarioDocument({
    mutationOptions: {
      onSuccess: (blob, variables) => {
        const fileName = editingVersion?.scenarioDocument ?? `scenario_doc_v${(variables as Record<string, unknown>).serviceVer}`;
        const url = window.URL.createObjectURL(blob as Blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = fileName;
