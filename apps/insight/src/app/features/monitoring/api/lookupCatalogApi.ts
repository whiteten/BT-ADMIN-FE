...
export const lookupCatalogApi = {
  ...
  updateLookupCatalog: async (lookupCatalogId: number, data: Partial<LookupCatalogCreateDatas>): Promise<LookupCatalogItem> => {
    const response = await apiClient.put<ApiResponse<LookupCatalogItem>>('/lookup-catalog-update', data, { params: { lookupCatalogId } });
    return response.data?.data;
  },

  deleteLookupCatalog: async (lookupCatalogId: number): Promise<void> => {
    await apiClient.delete('/lookup-catalog-delete', { params: { lookupCatalogId } });
  },
  ...
  getSchemaPreview: async (tableName: string): Promise<SchemaPreview> => {
    const response = await apiClient.get<ApiResponse<SchemaPreview>>('/lookup-catalog-schema-preview', { params: { tableName } });
    return response.data?.data;
  },
};
