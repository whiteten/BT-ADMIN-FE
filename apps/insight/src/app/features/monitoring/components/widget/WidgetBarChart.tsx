...
import { PANEL_PALETTE, axisLabelStyle, baseGrid, baseLegend, baseTooltip, splitLineStyle, verticalGradient } from '../../../panel/components/chart/echartsPanelTheme';
...
export default function WidgetBarChart({ detail, x, y, rows }: WidgetBarChartProps) {
  const option = useMemo(() => {
    ...
    const series = y.map((f, i) => {
      ...
      return {
        ...
        barMaxWidth: 36,
        itemStyle: { color: y.length === 1 ? verticalGradient(color) : color, borderRadius: [4, 4, 0, 0] },
    ...
    return {
      ...
      xAxis: { type: 'category', data: catKeys, axisLabel: axisLabelStyle, axisTick: { show: false } },
