...
import { DOMAIN_DOT_CLASS, DOMAIN_LABELS, VIZ_LABELS } from '../../constants/monitoringConstants';
...
import type { CustomWidgetCatalogItem, DomainCode, TemplateWidgetDefinitionListItem, VizType } from '../../types';
...
const DOMAIN_ICONS: Record<DomainCode, ReactNode> = {
  IE: <Cpu className="w-5 h-5" />,
  IC: <Monitor className="w-5 h-5" />,
  IR: <PhoneCall className="w-5 h-5" />,
};

const DOMAIN_ORDER: DomainCode[] = ['IE', 'IC', 'IR'];
...
export default function WidgetLibraryModal({ open, onClose, onAddTemplate, onAddCustom }: WidgetLibraryModalProps) {
  ...
  const filteredCustom = useMemo(
    () => (!kw ? customItems : customItems.filter((w) => w.widgetName.toLowerCase().includes(kw) || w.description?.toLowerCase().includes(kw))),
    [customItems, kw],
  );
  const filteredTemplate = useMemo(
    () =>
      !kw
        ? templateItems
        : templateItems.filter((t) => t.widgetName.toLowerCase().includes(kw) || t.description?.toLowerCase().includes(kw) || (t.datasetName ?? '').toLowerCase().includes(kw)),
    [templateItems, kw],
  );
  ...
  const groupByDomain = <T extends { domainCode: DomainCode }>(items: T[]): { domain: DomainCode; items: T[] }[] =>
    DOMAIN_ORDER.map((domain) => ({ domain, items: items.filter((i) => i.domainCode === domain) })).filter((g) => g.items.length > 0);

  const renderCard = (key: string, domain: DomainCode, name: string, description: string | undefined, meta: ReactNode, onClick: () => void) => (
    <button
      key={key}
      type="button"
      onClick={onClick}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="mt-4 flex items-center gap-2 flex-wrap">{meta}</div>
      </div>
  ...
  const domainHeader = (domain: DomainCode) => (
  ...
  const domainBadge = (domain: DomainCode) => (
    <span
  ...
  const renderGrid = () => {
    ...
    const groups = activeTab === 'custom' ? groupByDomain(filteredCustom) : groupByDomain(filteredTemplate);
    if (groups.length === 0) {
      return (
        <div className="h-full flex flex-col items-center justify-center text-center py-20">
          <Badge
            status="default"
            text={<span className="text-[14px] text-[#adb5bd]">{activeTab === 'custom' ? '등록된 커스텀 위젯이 없습니다.' : '등록된 템플릿 위젯이 없습니다.'}</span>}
          />
        </div>
      );
    }
    ...
    return groups.map((g) => (
      <section key={g.domain}>
        {domainHeader(g.domain)}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {activeTab === 'custom'
            ? (g.items as CustomWidgetCatalogItem[]).map((widget) =>
                renderCard(
                  widget.widgetTypeId,
                  widget.domainCode,
                  widget.widgetName,
            ...
            : (g.items as TemplateWidgetDefinitionListItem[]).map((tpl) =>
                renderCard(
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="bg-white px-8 pt-4 border-b border-[#f1f3f5]">
          <div className="flex items-center justify-between gap-6">
            <Tabs
              activeKey={activeTab}
              onChange={(k) => setActiveTab(k as 'custom' | 'template')}
        ...
        {/* 그리드 영역 */}
        <div className="flex-1 overflow-y-auto p-8 space-y-12 scrollbar-hide">{renderGrid()}</div>
