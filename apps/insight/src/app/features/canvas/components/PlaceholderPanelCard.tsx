...
interface PlaceholderPanelCardProps {
  /** "패널 추가하기" 버튼 클릭 시 호출 */
  onAdd: () => void;
  onDelete: () => void;
...
/**
 * 보고서 캔버스에 공간만 차지하는 빈 영역 카드. (모니터링 PlaceholderWidgetCard 와 동일)
 * "패널 추가하기" 클릭 시 패널 종류 선택 모달이 열린다.
 */
export default function PlaceholderPanelCard({ onAdd, onDelete, draggableClass = '' }: PlaceholderPanelCardProps) {
  return (
    <div className="group relative flex h-full flex-col overflow-hidden rounded-lg border-2 border-dashed border-[var(--color-bt-primary)]/20 bg-white/50 transition-all hover:border-[var(--color-bt-primary)]/40 hover:bg-white shadow-sm hover:shadow-md">
      ...
      <div className="flex flex-1 flex-col items-center justify-center p-6 text-center">
        ...
        >
          ...
          <span className="text-[14.5px] font-bold text-[#495057] group-hover/btn:text-[var(--color-bt-primary)] transition-colors">패널 추가하기</span>
        </button>
      </div>
    </div>
  );
