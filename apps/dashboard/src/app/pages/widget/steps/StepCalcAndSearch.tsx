...
import { useGetConditionList } from '../../../features/condition/hooks/useConditionQueries';
import { useValidateFormula } from '../../../features/widget/hooks/useWidgetQueries';
...
export default function StepCalcAndSearch({ calcFields, onCalcFieldsChange, searchBindings, onSearchBindingsChange, selectedDatasourceKeys, availableFields }: Props) {
  ...
  const removeCalcField = (index: number) => {
    onCalcFieldsChange(calcFields.filter((_, i) => i !== index));
  };

  const validateFormula = (index: number) => {
    const formula = calcFields[index].formula;
  ...
  const removeSearchBind = (index: number) => {
    onSearchBindingsChange(searchBindings.filter((_, i) => i !== index));
  };
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-sm text-gray-500">데이터소스에 없는 필드를 수식으로 생성합니다. {'{필드명}'}으로 기존 필드를 참조합니다.</p>
          </div>
            ...
            <Card key={idx} size="small" className="border-l-4 border-l-purple-500">
              <div className="space-y-3">
                ...
                <div className="flex gap-2 text-xs">
                  ...
                  {['SUM', 'AVG', 'IF', 'ROUND', 'RATIO', 'RANK', 'OFFSET', 'RUNNING_SUM', 'MOVING_AVG'].map((f) => (
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-sm text-gray-500">기존에 등록된 검색조건을 선택하여 데이터소스 필드에 바인딩합니다.</p>
          </div>
              ...
              render: (_: unknown, __: SearchBind, idx: number) => (
                <Select
                  ...
                  options={conditions.map((c) => ({
                    value: c.conditionId,
                    label: `${c.conditionName} (${c.inputType})`,
                  }))}
                  placeholder="검색조건 선택"
                  style={{ width: '100%' }}
                />
