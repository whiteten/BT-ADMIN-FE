...
export const bffFlowApi = {
  ...
  getFlows: async (): Promise<BffFlow[]> => {
    const response = await apiClient.get<ListResponse<BffFlow>>('/bff-flow-list');
    return extractList(response);
  },

  /** Flow 상세 조회 */
  getFlow: async (flowId: string): Promise<BffFlow> => {
    const response = await apiClient.get<{ data: BffFlow }>('/bff-flow-detail', {
      params: { flowId },
    });
  ...
  saveFlow: async ({ flowId, spec }: { flowId: string; spec: FlowSpec }): Promise<BffFlow> => {
    const response = await apiClient.post<{ data: BffFlow }>('/bff-flow-create', spec, {
      params: { flowId },
    });
    return response?.data?.data;
  },
