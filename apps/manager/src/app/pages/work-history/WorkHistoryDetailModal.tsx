...
export interface WorkHistoryDetailModalRef {
...
export const WorkHistoryDetailModal = forwardRef<WorkHistoryDetailModalRef>(function WorkHistoryDetailModal(_, ref) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="sm:max-w-[900px] h-[700px] overflow-hidden flex flex-col">
        <DialogHeader className="pb-4 border-b flex-shrink-0">
          <DialogTitle className="flex items-center gap-3">
            {detail?.master && (
              <span className={`flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium ${statusConfig[detail.master.status]?.className || 'bg-gray-100'}`}>
                {statusConfig[detail.master.status]?.icon}
                {statusConfig[detail.master.status]?.label}
              </span>
            )}
            <span>작업 상세 정보</span>
          </DialogTitle>
...
function ModalContent({ detail }: { detail: WorkHistoryDetail }) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex-1 min-h-0">
        <Tabs
          ...
          className="h-full [&_.ant-tabs-content]:h-[280px] [&_.ant-tabs-tabpane]:h-full [&_.ant-tabs-tabpane]:overflow-auto"
          items={[
            {
              key: 'apiCalls',
              label: (
