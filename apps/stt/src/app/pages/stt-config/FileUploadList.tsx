  ...
  const columnDefs: ColDef<FileUploadItem>[] = [
    {
      ...
      field: 'callDate',
      flex: 2,
      valueFormatter: ({ value }: { value: string }) => (value ? dayjs(value, 'YYYYMMDD').format('YYYY-MM-DD') : ''),
    },
    {
      headerName: '등록시',
      field: 'callTime',
      flex: 2,
      valueFormatter: ({ value }: { value: string }) => (value ? dayjs(value, 'HHmmss').format('HH:mm:ss') : ''),
    },
