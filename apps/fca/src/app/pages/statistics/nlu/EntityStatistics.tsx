...
export default function EntityStatistics() {
  ...
  const filteredList = useMemo(() => {
    ...
    if (scnIds.length === 0 && modelIds.length === 0) return entityStatList;
    return entityStatList.filter((entityStat) => {
      const matchesScn = !scnIds.length || scnIds.includes(String(entityStat.scnId ?? ''));
      const matchesModel = !modelIds.length || modelIds.includes(String(entityStat.modelId ?? ''));
      return matchesScn && matchesModel;
    });
  }, [entityStatList, scnIds, modelIds]);
