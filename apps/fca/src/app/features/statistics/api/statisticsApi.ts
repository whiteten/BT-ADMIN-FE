...
export const statisticsApi = {
  ...
  getServiceStatList: async (params?: Record<string, unknown>): Promise<ServiceStatListItem[]> => {
    const response = await apiClient.get<ListResponse<ServiceStatListItem>>('/stat-bot-service', { params });
    return extractList(response);
  },

  // 대화 통계 목록 조회
  getDialogStatList: async (params?: Record<string, unknown>): Promise<DialogStatListItem[]> => {
    const response = await apiClient.get<ListResponse<DialogStatListItem>>('/stat-bot-dialog', { params });
    return extractList(response);
  },

  // 슬롯 통계 목록 조회
  getSlotStatList: async (params?: Record<string, unknown>): Promise<SlotStatListItem[]> => {
    const response = await apiClient.get<ListResponse<SlotStatListItem>>('/stat-bot-slot', { params });
    return extractList(response);
  },

  // 의도 통계 목록 조회
  getIntentStatList: async (params?: Record<string, unknown>): Promise<IntentStatListItem[]> => {
    const response = await apiClient.get<ListResponse<IntentStatListItem>>('/stat-nlu-intent', { params });
    return extractList(response);
  },

  // 개체 통계 목록 조회
  getEntityStatList: async (params?: Record<string, unknown>): Promise<EntityStatListItem[]> => {
    const response = await apiClient.get<ListResponse<EntityStatListItem>>('/stat-nlu-entity', { params });
    return extractList(response);
  },

  // 키워드 통계 목록 조회
  getKeywordStatList: async (params?: Record<string, unknown>): Promise<KeywordStatListItem[]> => {
    const response = await apiClient.get<ListResponse<KeywordStatListItem>>('/stat-nlu-keyword', { params });
    return extractList(response);
  },
};
