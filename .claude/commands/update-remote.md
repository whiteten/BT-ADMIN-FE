---
description: 기존 remote 앱을 점검해 remote 생성기(turbo gen) 기준으로 누락된 파일·구문을 보강하고 menu-config 잔재 제거 및 뱃지 아이콘 등록 누락을 점검
...
`pnpm gen remote`(turbo gen) 생성기로 생성됐어야 할 파일이나 host 측 등록 구문이 누락된 remote 앱을 사후 정상화합니다. 기준 생성기: [turbo/generators/config.ts](../../turbo/generators/config.ts) + 템플릿 [turbo/generators/templates/remote/](../../turbo/generators/templates/remote/).

## SoT 원칙 — 점검 항목은 remote 생성기가 단일 출처

이 커맨드는 점검 항목 표를 따로 들고 있지 않습니다. **매 실행마다 [turbo/generators/config.ts](../../turbo/generators/config.ts)를 직접 읽고**, `remote` 생성기의 `actions` 배열을 순서대로 분석해 동일한 작업(파일 생성·등록 지점 패치)이 "이미 되어 있는지" 검사하는 방식으로 동작합니다. 생성기가 수정되면 별도 동기화 없이 자동으로 반영됩니다.
...
> **`custom` 제외 사유**: `apps/custom`은 현장 커스텀 오버라이드 운반체로 **일반 업무 remote가 아니다**. 생성기 표준 산출물과 의도적으로 다르게 구성되어 있어(host remotes 배열 미등록, consume-only shared, `features/router/` 없음, exposes가 라우트가 아닌 오버라이드 키) 이 커맨드의 정규화 기준을 적용하면 오히려 구조가 깨진다. 상세는 [CUSTOM_DEVELOPMENT_GUIDE.md](../../doc/CUSTOM_DEVELOPMENT_GUIDE.md) 참조.

## 2. remote 생성기 분석 — 점검 항목 도출

[turbo/generators/config.ts](../../turbo/generators/config.ts)를 읽고 `remote` 생성기의 `actions` 배열을 위에서 아래로 따라가며, 각 액션이 **어떤 파일에 어떤 변경을 가하는지** 파악한다. 각 액션마다 점검 항목 한 개를 만들고 TodoWrite로 등록한다.

액션 분석 시 다음 사항을 기준으로 점검 로직을 도출한다:

- **`add` 액션** (templates/remote/의 `.hbs` 템플릿으로 파일 생성): 대상 파일이 존재하고, 템플릿에 `{{name}}` 등 변수를 치환한 결과와 구조적으로 일치하는지 확인. 누락이면 템플릿 기준으로 생성.
- **`modify` 액션** (등록 지점 패치 — `tools/mf/app-ports.ts`의 APP_PORTS·REMOTE_NAMES, `scripts/serve-host.js`의 REMOTE_APPS, host 로더 3종의 ROUTE/VARIANT/SELECTOR_LOADERS, `apps/host/src/remotes.d.ts` 와일드카드 선언): 해당 항목이 이미 존재하는지 grep으로 확인. 없으면 액션과 동일한 패턴으로 삽입.
- **커스텀 액션** (예: `copyFavicon`): 대상 파일(`src/favicon.ico`) 존재 여부 확인. 없으면 manager 것 복사.
- **JSON 파일**(package.json 등)은 생성기 템플릿이 작성하는 필드만 확인 (사용자가 추가한 다른 필드·의존성은 건드리지 않는다).

> 액션 타입·경로·템플릿 파일명이 명확해 별도 표 없이도 분석 가능. 분석 결과가 모호하면 사용자에게 확인을 요청한다.
...
## 3. 생성기에 없는 정리 작업 — 잔재 제거 및 컨벤션 점검

remote 생성기는 신규 생성용이라 과거 잔재 제거나 코딩 컨벤션 점검 로직이 없다. 이 항목은 이 커맨드가 별도로 처리한다. 각 항목마다 TodoWrite 항목을 따로 등록한다.
...
remote 생성기는 remote에 main 페이지를 만들지 않는다(구 create-remote 시절 정책 변경 유지). 메인 페이지는 host의 `/`가 단독으로 제공하며, 모든 remote의 루트(`/<APP_NAME>`) 진입과 404 fallback은 host `/`로 redirect된다. 기존 remote에 남아있는 다음 잔재를 정리한다.
...
remote 생성기는 골격만 생성하므로, 이후 운영하며 추가된 페이지·feature가 프로젝트 표준에서 벗어났는지는 점검하지 않는다. 이 항목은 대상 remote 전체를 **CLAUDE.md 기준**으로 점검한다.
...
> 컨벤션 위반은 빌드를 깨뜨리지 않는 경우가 많아 자동 수정 시 무리한 변경이 될 수 있다. 반드시 사용자 확인을 거치고, 변경 후 영향받은 모든 파일에 `npx eslint --fix`를 실행한다(마무리 섹션의 일괄 처리에 포함). rename·이동 후에는 깨진 import가 없는지 `npx tsc -p apps/<APP_NAME>/tsconfig.app.json --noEmit`로 검증한다.

## 4. tsconfig.base.json 정리

`compilerOptions.paths`에 `<APP_NAME>/Module` 키가 남아있으면 제거한다(구 create-remote 시절 잔재 — 현 생성기는 paths를 건드리지 않음). 제거 후 `npx prettier --write tsconfig.base.json` 실행.
...
- 생성기 분석 중 해석이 모호하거나 예상치 못한 파일 상태(예: 템플릿과 전혀 다른 구조)를 발견하면 즉시 중단하고 사용자에게 보고한다.
