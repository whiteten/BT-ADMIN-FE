...
**remote 목록은 하드코딩하지 않는다**: 어떤 remote가 있는지는 `apps/` 디렉토리에서 동적으로 발견한다(`apps/<app>/src/app/routes.tsx`가 있는 디렉토리 = remote, host와 custom은 자동 제외 — custom은 현장 커스텀 오버라이드 운반체라 업무 화면이 없음). 선택지·검증·기본 대상 모두 이 동적 목록을 SoT로 쓴다. 명령은 `node .claude/skills/generate-manual/scripts/extract-routes.mjs --list-apps`(줄바꿈) 또는 `--list-apps --json`. 신규 remote가 추가되면 스킬·스크립트 수정 없이 자동으로 목록에 포함된다.
