---
name: react-perf
description: React 성능 점검·리팩터 체크리스트. 번들(barrel import·lazy), async 워터폴(Promise.all), 파생상태(useEffect 오용 제거), 리렌더, 알고리즘(Set/Map) 룰을 이 저장소 맥락(React Compiler·Rsbuild MF·client-only SPA)으로 정리. "성능 점검", "느려요", "리렌더 너무 많음", "번들 큼", "리팩터 성능" 시 사용. 출처: Vercel agent-skills(MIT)에서 stack-agnostic 룰만 발췌·재작성.
---
...
- **Rsbuild 2(rspack) + Module Federation, client-only SPA** → Next.js·RSC·SSR·hydration 룰은 미적용이라 제외. lazy-load는 `React.lazy`(routes.tsx에서 이미 사용).
...
> 출처: [Vercel agent-skills `react-best-practices`](https://github.com/vercel-labs/agent-skills) (MIT). 70룰 중 이 저장소 스택(React Compiler·Rsbuild MF·client-only)에 적용되는 룰만 발췌·재작성. RSC/server·SWR·수동 메모 룰은 제외.
