---
name: add-tab-bar
description: 노드/카테고리 탭 바(좌우 스크롤 화살표 + 가로 스크롤 탭 스트립 + 우측 검색·액션) 작성 절차. "탭바 + 카드 슬라이더 + 하단 그리드" 3단 목록 화면의 1단. 공용 컴포넌트 NodeTabBar(libs/shared-ui) 사용. 레퍼런스 IvrDnGroupList.tsx, IvrMedia.tsx(NodeTabBar 적용), HaGroupList.tsx, IvrEndpointList.tsx(마이그레이션 전 — 인라인 마크업). 2단(카드 슬라이더)은 add-card-slider, 3단(하단 그리드)은 add-grid 스킬 참조. "탭바 만들어줘", "노드 탭 추가", "카테고리 탭 스트립" 요청 시 사용.
---
...
> ⚠️ **인라인 마크업으로 새로 작성하지 말 것.** 과거 화면마다 탭바 마크업을 복붙하다 보니 패딩·최대폭·구분선 등이 화면마다 미세하게 어긋나는 문제가 반복됐다(2026-07 IVR DN그룹관리 vs 미디어관리 드리프트). 반드시 공용 컴포넌트 **`NodeTabBar`**(`@/components/custom/NodeTabBar`, 실체는 `libs/shared-ui/src/components/custom/NodeTabBar.tsx`)를 사용해 두 화면이 항상 같은 결과물을 내도록 한다.

레퍼런스(NodeTabBar 적용): [IvrDnGroupList.tsx](../../../apps/ivr/src/app/pages/line/IvrDnGroupList.tsx), [IvrMedia.tsx](../../../apps/ivr/src/app/pages/line/IvrMedia.tsx).

> [HaGroupList.tsx](../../../apps/ivr/src/app/pages/ha/HaGroupList.tsx), [IvrEndpointList.tsx](../../../apps/ivr/src/app/pages/line/IvrEndpointList.tsx)는 아직 인라인 마크업(마이그레이션 전)이다. 이 두 화면을 수정할 일이 생기면 겸사겸사 `NodeTabBar`로 옮길 것 — 새로 인라인 코드를 늘리지 말 것.
...
| 상위 항목이 소수(2~4개)라 스크롤이 필요 없는 단순 탭 | 골격은 동일하되 `NodeTabBar` 그대로 사용(좌우 화살표는 항목이 적어도 그대로 둬도 무방 — 클릭해도 스크롤할 게 없을 뿐) |
...
1. **컴포넌트 사용**: 탭바를 새로 만들 때 마크업을 손으로 복붙하지 말고 `NodeTabBar`를 import해서 쓴다. 좌우 스크롤 화살표, `h-[56px]` 박스, 탭 버튼 스타일(`px-3` + `min-w-[120px] max-w-[200px]`), 활성/비활성 색상, `(N)` 카운트 포맷은 컴포넌트 내부에 고정되어 있어 화면마다 달라질 수 없다.
...
- [ ] `NodeTabBar`를 import해서 썼는가(마크업을 손으로 복붙하지 않았는가)?
