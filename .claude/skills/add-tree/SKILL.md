...
3. 행 chrome(들여쓰기·선택바·라벨 톤)은 `TreeRow`/`TreeCaret`/`TreeFolderIcon`/`TreeLabel` 프리미티브를 사용한다. 톤 기준 SoT는 상담사 설정 `AgentGroupTree`.
4. 선택·DnD·아이콘·액션·카운트 등 도메인 결합 로직은 소비처가 주입한다 — 훅·프리미티브에 도메인 로직을 넣지 않는다.
5. hover 액션이 있는 트리는 **행 높이 고정 규격**(아래 "카운트 + hover 액션" 참조)을 반드시 적용한다 — 미적용 시 hover 때 행이 흔들린다.
...
### 카운트 + hover 액션 (행 높이 고정 규격)

평소엔 카운트가 맨 우측, hover 시 카운트가 숨고 그 자리에 액션 버튼. **양쪽 모두 `h-5`로 높이를 맞춰** 행 높이가 흔들리지 않게 한다:

```tsx
{/* 카운트 — hover 시 숨김 */}
<span className="h-5 inline-flex items-center text-[11px] text-gray-400 flex-shrink-0 group-hover:hidden">
  {node.count.toLocaleString()}
</span>
{/* 액션 — 기본 숨김(자리 차지 안 함), hover 시에만 */}
<div className="hidden group-hover:flex items-center gap-0.5 flex-shrink-0">...버튼들...</div>
```

**카운트가 없는 트리**는 모든 행에 `h-5` 고정 슬롯을 상시 렌더하고 버튼만 `hidden group-hover:inline-flex` 처리 (레퍼런스: RecogGroupTree, MenuTree):

```tsx
<div className="flex items-center gap-0.5 h-5 flex-shrink-0">
  {hasActions && <button className="hidden group-hover:inline-flex w-5 h-5 ..." />}
</div>
```
...
- 툴팁: antd `Tooltip` + 컴팩트 규격 `TOOLTIP_PROPS`(AgentGroupTree 참조 — mouseEnterDelay 0.5, styles.container로 min-height 제거). **원본/기획에 툴팁이 없던 화면에 임의로 추가하지 않는다.**
...
| `apps/manager/.../menu/components/MenuTree.tsx` | 도메인 아이콘·가상 루트·검색 Highlight·필터 칩 Popover |
...
| `apps/ipron/.../agent-master/components/AgentGroupTree.tsx` | **풀기능 SoT** — 외부 DnD + 노드 드래그 reorder + read-only 모드 + 툴팁 규격 |
